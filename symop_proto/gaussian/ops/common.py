...
def as_index_list(
    core: GaussianCore, modes: Sequence[ModeOpProto]
) -> List[int]:
    return [core.basis.require_index_of(m) for m in modes]


def check_is_square_matrix(
    X: np.ndarray, *, name: Optional[str] = None
) -> None:
    X = np.asarray(X)
...
def check_is_same_shape(U: np.ndarray, V: np.ndarray) -> None:
    if U.shape != V.shape:
        raise ValueError(
            f"U and V must have same shapes, got {U.shape} vs {V.shape}"
        )
...
def check_ccr_preservation(
    ...
    if not np.allclose(lhs1, G, atol=atol, rtol=0.0):
        ...
        raise ValueError(
            f"Bogoliubov map violates [a,a^dag]=G, max abs error: {err}"
        )
    ...
    if not np.allclose(lhs2, 0.0, atol=atol, rtol=0.0):
        ...
        raise ValueError(
            f"Bogoliubov map violates [a,a]=0; max abs error: {err}"
        )
...
def check_finite_all(
    X: np.ndarray, Y: np.ndarray, d0: Optional[np.ndarray]
) -> None:
