...
def _make_output_envelope(
    env_in: object, transfer: SpectralTransfer
) -> object:
    if isinstance(env_in, GaussianEnvelope) and isinstance(
...
class PathSpectralFilter(GaussianDevice):
    ...
    def resolve_io(self, state: GaussianCore) -> DeviceIO:
        ...
        for m in modes_in:
            ...
            if in_lbl is None:
                raise ValueError(
                    "PathSpectralFilter expects modes with a .label"
                )
    ...
    def do_apply(self, state: GaussianCore, io: DeviceIO) -> GaussianCore:
        ...
        def _assert_finite(name: str, A: np.ndarray) -> None:
            if not np.isfinite(A).all():
                ...
                raise ValueError(
                    f"{name} has non-finite at {(i, j)}: {A[i, j]!r}"
                )
