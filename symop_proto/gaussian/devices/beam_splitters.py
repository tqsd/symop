...
def _env_key(
    env: Any, *, approx: bool, approx_kw: Dict[str, Any]
) -> Tuple[Any, ...]:
    if not approx:
...
class PathBeamSplitter(GaussianDevice):
    ...
    def resolve_io(self, state: GaussianCore) -> DeviceIO:
        ...
        if set(modes1).intersection(modes2):
            raise ValueError(
                "PathBeamSplitter requires disjoint port mode sets"
            )
        ...
        for k in sorted(all_keys, key=repr):
            ...
            if len(a) > n:
                if not self.vacuum_fill:
                    raise ValueError(
                        f"Unpaired modes on port1 and vacuum_fill=False. key={
                            k!r}"
                    )
            ...
            if len(b) > n:
                if not self.vacuum_fill:
                    raise ValueError(
                        f"Unpaired modes on port2 and vacuum_fill=False. key={
                            k!r}"
                    )
        ...
        if out1 is None or out2 is None:
            raise ValueError(
                "Internal error: out_path1/out_path2 not initialized"
            )
    ...
    def do_apply(self, state: GaussianCore, io: DeviceIO) -> GaussianCore:
        ...
        for i1, i2 in pairs:
            ...
            if m1 is None:
                if not self.vacuum_fill:
                    raise ValueError(
                        "Missing port1 partner with vacuum_fill=False"
                    )
            ...
            if m2 is None:
                if not self.vacuum_fill:
                    raise ValueError(
                        "Missing port2 partner with vacuum_fill=False"
                    )
        ...
        if k % 2 != 0:
            raise ValueError(
                "Internal error: acted subset must have even length"
            )
        ...
        if env_modes:
            ...
            object.__setattr__(
                io, "meta", meta
            )  # DeviceIO may be frozen; if so, skip.
