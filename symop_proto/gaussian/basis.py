...
class ModeBasis:
    ...
    def __post_init__(self) -> None:
        ...
        if self.gram.shape != (n, n):
            raise ValueError(
                f"gram matrix must have shape ({n},{n}), "
                f"got {self.gram.shape}"
            )
        ...
        if idxs != list(range(n)):
            raise ValueError(
                "index_by_sig values must be a permutation of 0..n-1"
            )
        ...
        *,
        ...
        for m in modes:
            key = (
                m.signature
                if not merge_approx
                else m.approx_signature(**env_kw)
            )
    ...
    def validate(
        ...
        if psd and not self.is_positive_semidefinite(eps=eps):
            raise ValueError(
                "gram is not positive semidefinite within tolerance"
            )
