...
class GaussianEnvelope(BaseEnvelope):
    ...
    Frequency domain
    ----------------
    With the Fourier convention used elsewhere in this package:
    ...
    .. math::
        ...
        \exp\!\bigl(i(\omega+\omega_0)\tau + i\phi_0\bigr),
    ...
    def omega_sigma(self) -> float:
        # Spectral width hint in rad/s. For a Gaussian, sigma_w ~ 1/(2*sigma_t) or 1/sigma_t
        # depending on whether you define exp(-(t^2)/(4*sigma_t^2)) as you do.
        # Your time amplitude is exp(-(t^2)/(4*sigma_t^2)) -> variance in time is 2*sigma_t^2?:
        # Actually |z|^2 has exp(-(t^2)/(2*sigma_t^2)), so sigma_t is the intensity std dev.
        # Then the corresponding intensity spectral std dev is 1/sigma_t.
        s = float(self.sigma)
    ...
    def time_eval(self, t: FloatArray) -> RCArray:
        ...
        if not (s > 0.0) or not np.isfinite(s):
            raise ValueError(
                f"sigma must be positive finite, got {self.sigma!r}"
            )
    ...
    def freq_eval(self, w: FloatArray) -> RCArray:
        ...
        # z(t) = N exp(-(t-tau)^2/(4 s^2)) exp(i(omega0(t-tau)+phi0))
        # With inverse convention z(t) = (1/2pi) ∫ Z(w) exp(+i w t) dw,
        # we get (up to a constant) Z(w) ∝ exp(-s^2 (w-omega0)^2) * exp(-i w tau) * exp(i phi0)

        N = (1.0 / (2.0 * np.pi * s * s)) ** 0.25
        A = N * (
            2.0 * s * np.sqrt(np.pi)
        )  # consistent constant (does not need to be exact)
    ...
    def overlap(self, other: EnvelopeProto) -> complex:
        # keep your existing closed form exactly as-is
