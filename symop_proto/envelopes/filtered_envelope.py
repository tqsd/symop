...
def _remove_linear_phase(
    w_rel: np.ndarray, Z: np.ndarray, t0: float
) -> np.ndarray:
...
class FilteredEnvelope(BaseEnvelope):
    ...
    def approx_signature(
        ...
        **kw: Any,
        ...
        return (
            "filtered_approx",
            getattr(
                self.base, "approx_signature", lambda **_: self.base.signature
    ...
    def center_and_scale(self) -> Tuple[float, float]:
        ...
        z = (
            np.fft.fftshift(np.fft.ifft(np.fft.ifftshift(Z)))
            * (n * dw)
            / (2.0 * np.pi)
        )
    ...
    def freq_eval(self, w: FloatArray) -> RCArray:
        ...
        if not np.isfinite(Zw).all():
            ...
            i = int(bad[0, 0])
            raise ValueError(
                f"Non-finite base.freq_eval(w) at w={w[i]}: {Zw[i]!r}"
            )
    ...
    def delayed(self, dt: float) -> "FilteredEnvelope":
        base_delayed = (
            self.base.delayed(dt)
            if hasattr(self.base, "delayed")
            else self.base
        )
    ...
    def phased(self, dphi: float) -> "FilteredEnvelope":
        base_phased = (
            self.base.phased(dphi)
            if hasattr(self.base, "phased")
            else self.base
        )
        return FilteredEnvelope(
            base=base_phased,
    ...
    def overlap_with_generic(self, other: EnvelopeProto) -> complex:
        ...
        if not np.isfinite(z1).all():
            ...
            i = int(bad[0, 0])
            raise ValueError(
                f"Non-finite self.freq_eval at w={w[i]}: {z1[i]!r}"
            )
        if not np.isfinite(z2).all():
            bad = np.argwhere(~np.isfinite(z2))
            i = int(bad[0, 0])
            raise ValueError(
                f"Non-finite other.freq_eval at w={w[i]}: {z2[i]!r}"
            )
