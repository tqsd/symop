...
def _match_inv_sqrt(
    v: float, tol: float = 1e-12, nmax: int = 16
) -> int | None:
    if not isfinite(v):
        return None
...
def complex_to_text(c: complex, *, tol: float = 1e-12) -> str:
    ...
    b_txt = (
        ...
        else scalar_to_text(b_abs, tol=tol) + "·i"
    )
    return f"{scalar_to_text(a, tol=tol)}{sgn}{b_txt}"


def scalar_to_latex(
    x: float, *, tol: float = 1e-12, use_tfrac: bool = False
) -> str:
...
def complex_to_latex(
    c: complex, *, tol: float = 1e-12, use_tfrac: bool = False
) -> str:
    a = 0.0 if abs(c.real) < tol else c.real
    b = 0.0 if abs(c.imag) < tol else c.imag
    if b == 0.0:
    ...
    if a == 0.0:
        ...
        if _approx_int(b, tol) == -1:
            return r"-\mathrm{i}"
        return (
            scalar_to_latex(b, tol=tol, use_tfrac=use_tfrac) + r"\,\mathrm{i}"
        )
    sign = "+" if b > 0 else "-"
    b_abs = abs(b)
    b_tex = (
        r"\mathrm{i}"
        if _approx_int(b_abs, tol) == 1
        else scalar_to_latex(b_abs, tol=tol, use_tfrac=use_tfrac)
        + r"\,\mathrm{i}"
    )
