...
class KetPoly(KetPolyProto):
    ...
    def from_ops(
        *,
        ...
        return KetPoly(
            ket_from_ops(
                creators=creators, annihilators=annihilators, coeff=coeff
            )
        )
    ...
    def is_creator_only(self) -> bool:
        ...
        return all(
            t.monomial.is_creator_only or t.monomial.is_identity
            for t in self.terms
        )
    ...
    def is_annihilator_only(self) -> bool:
        return all(
            t.monomial.is_annihilator_only or t.monomial.is_identity
            for t in self.terms
        )
...
class DensityPoly(DensityPolyProto):
    ...
    def is_creator_only_left(self) -> bool:
        return all(
            dt.left.is_creator_only or dt.left.is_identity for dt in self.terms
        )

    @property
    def is_creator_only_right(self) -> bool:
        return all(
            dt.right.is_creator_only or dt.right.is_identity
    ...
    def is_diagonal_in_monomials(self) -> bool:
        return all(
            dt.left.signature == dt.right.signature for dt in self.terms
        )
