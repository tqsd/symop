...
class TestGaussianHighpass(unittest.TestCase):
    def test_signature(self):
        tf = GaussianHighpass(w0=1.5, sigma_w=2.0)
        self.assertEqual(tf.signature, ("gauss_highpass", 1.5, 2.0))

    def test_approx_signature_rounds(self):
        tf = GaussianHighpass(w0=1.234567, sigma_w=9.876543)
    ...
    def test_call_matches_definition(self):
        w0 = 10.0
        sigma = 2.0
        tf = GaussianHighpass(w0=w0, sigma_w=sigma)

        w = np.array([w0, w0 + sigma, w0 + 2.0 * sigma], dtype=float)
        out = tf(w)

        expected = (1.0 - np.exp(-0.5 * ((w - w0) / sigma) ** 2)).astype(complex)
    ...
    def test_call_accepts_python_sequence(self):
        tf = GaussianHighpass(w0=0.0, sigma_w=1.0)
        out = tf([0.0, 1.0, 2.0])

        w = np.array([0.0, 1.0, 2.0], dtype=float)
        expected = (1.0 - np.exp(-0.5 * w * w)).astype(complex)
        np.testing.assert_allclose(out, expected)
    ...
    def test_call_range_bounds(self):
        tf = GaussianHighpass(w0=0.0, sigma_w=1.0)
        w = np.linspace(-10.0, 10.0, 401)
        out = tf(w).real

        self.assertTrue(np.all(out >= -1e-15))
        self.assertTrue(np.all(out <= 1.0 + 1e-15))

        # At the center, high-pass is minimal: H(w0) = 0
        center = tf(np.array([0.0], dtype=float)).real[0]
        self.assertAlmostEqual(center, 0.0, places=14)

        # Far away, it approaches 1
        far = tf(np.array([100.0], dtype=float)).real[0]
        self.assertAlmostEqual(far, 1.0, places=14)
    ...
    def test_call_raises_on_nonpositive_sigma(self):
        tf0 = GaussianHighpass(w0=0.0, sigma_w=0.0)
        with self.assertRaises(ValueError):
            _ = tf0([0.0, 1.0])

        tfneg = GaussianHighpass(w0=0.0, sigma_w=-1.0)
        with self.assertRaises(ValueError):
            _ = tfneg([0.0, 1.0])

    def test_call_raises_on_nonfinite_sigma(self):
        tfnan = GaussianHighpass(w0=0.0, sigma_w=float("nan"))
        with self.assertRaises(ValueError):
            _ = tfnan([0.0, 1.0])

        tfinf = GaussianHighpass(w0=0.0, sigma_w=float("inf"))
        with self.assertRaises(ValueError):
            _ = tfinf([0.0, 1.0])

    def test_call_does_not_modify_input(self):
        tf = GaussianHighpass(w0=1.0, sigma_w=2.0)
