...
from symop_proto.algebra.density.apply_right import density_apply_right
...
class TestDensityApplyRight(ExtendedTestCase):
    ...
    def test_single_annihilator_on_right_identity(self):
        m = make_mode("A")
        rho = (DensityTerm(3.0, left=Monomial(), right=Monomial()),)
        out = density_apply_right(rho, word=(m.ann,))
    ...
    def test_creator_against_existing_creator_on_right(self):
        ...
        self.assertComplexAlmostEqual(coeff_by_sig[adag_a_sig], 1.0 + 0j)

        # Left monomial remains unchanged
        for t in out:
            self.assertEqual(t.left.signature, Monomial().signature)
    ...
    def test_word_can_be_generator(self):
        ...
        word_gen = (op for op in (m.ann,))  # generator
        out = density_apply_right(rho, word=word_gen)
        self.assertEqual(len(out), 1)
        self.assertEqual(
            out[0].right.signature, Monomial(creators=(m.create,)).signature
        )
