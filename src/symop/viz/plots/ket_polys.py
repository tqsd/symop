r"""Plotting for polynomial ket operators.

This module provides a plotting dispatcher implementation for
:class:`KetPoly`. The visualization focuses on the temporal and
spectral envelopes associated with the unique modes appearing in the
polynomial.

For each unique mode with an associated envelope, two plots are
produced:

- time-domain magnitude :math:`|\zeta(t)|`
- frequency-domain magnitude :math:`|Z(\omega)|`

The plotting intentionally deduplicates modes so that shared envelopes
are shown only once.
"""
...
def _estimate_freq_window(env: Any, *, w_span_sigma: float) -> tuple[float, float]:
    ...
    w_span_sigma:
        Half-width multiplier for the frequency window, expressed in
        units of spectral width.

    Returns
    -------
    tuple of float
        Pair ``(w0, W)`` where ``w0`` is the center frequency and
        ``W`` is the half-width of the plotted frequency range.
...
def _default_mode_tag(mode: Any) -> str:
    ...
    -------
    str
        Human-readable mode tag. Returns an empty string if no suitable
        label is available.
...
@plot.register(KetPoly)
def _plot_ket_poly(obj: KetPoly, /, **kwargs: Any) -> Any:
    """Plot envelopes referenced by a :class:`KetPoly`.

    The plotting routine extracts the unique modes appearing in the ket
    polynomial and visualizes their corresponding envelopes in the time
    and frequency domains.

    Parameters
    ----------
    obj:
        Ket polynomial to visualize.
    **kwargs:
    ...
    Notes
    -----
    This function intentionally plots only the unique modes of the
    polynomial, in first-seen order, to avoid duplicate subplots when
    multiple terms share the same mode.

    If the polynomial contains no modes, a minimal placeholder figure
    labeled ``"zero ket"`` is returned.
    ...
    if len(modes) == 0:
        ...
        ax.text(0.5, 0.5, "zero ket", ha="center", va="center")
        ax.set_axis_off()
