...
class GaussianDevice(BaseDevice[GaussianCore]):
    ...
    def _keep(self, state: GaussianCore, modes: Sequence[ModeOpProto]) -> GaussianCore:
    ...
    def _relabel(
        ...
        *,
        ...
        if len(set(sigs)) != len(sigs):
            raise ValueError("Relabel produced duplicate modes (signature collision)")

        new_basis = ModeBasis.build(tuple(new_modes))

        if not np.allclose(new_basis.gram, old_basis.gram, atol=atol, rtol=0.0):
            raise ValueError(
